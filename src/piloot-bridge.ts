...
function следитьЗаТемойИЧатом() {
   ...
   *
   * Берём только левую колонку — список, архив, поиск. Ту же примету
   * носят аватарки и фотографии в переписке: нажатие по ним чат не
   * меняет, и панель от них дёргаться не должна.
   *
   * Номер лежит не на самой строке, а внутри неё, на названии
   * собеседника (`.peer-title[data-peer-id]`) — этим же отбором
   * пользуется сам Web K, когда правит названия.
   *
  ...
  document.addEventListener('click', (event) => {
    ...
    if(!цель?.closest?.('#column-left')) return;

    const строка = цель.closest('.row') as HTMLElement | null;
    const носитель = (строка?.querySelector('[data-peer-id]') ??
      цель.closest('[data-peer-id]')) as HTMLElement | null;
    const номер = носитель?.dataset?.peerId;
...
/*
 * «В сети» (Piloot 207). Web K ставит «не в сети», когда его окно получает
 * событие «потерял фокус», а «в сети» — на первое движение мыши над ним и
 * на фокус (helpers/idleController.ts). В Piloot он живет в рамке: когда
 * мак переключается на другую программу, а фокус был у панели, рамка этого
 * события не получает, и Web K шлет «я в сети» раз в ~52 с все время, пока
 * Piloot открыт (замер 207: 60–67 раз в час против 0 у чистого Web K).
 *
 * Окно Piloot потеряло фокус целиком — говорим Web K то же, его же
 * событием на его окне. Разметку и файлы Web K не трогаем (правило 12).
 * Фокус ушел из панели в саму рамку — это не уход: окно Piloot в фокусе.
 */
function следитьЗаФокусомОкна() {
  let родитель: Window;
  try {
    родитель = window.parent;
    void родитель.document;
  } catch {
    return;
  }

  родитель.addEventListener('blur', () => {
    setTimeout(() => {
      try {
        if(!родитель.document.hasFocus()) window.dispatchEvent(new Event('blur'));
      } catch {}
    }, 0);
  });
}
